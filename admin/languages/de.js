/**
 * German translation for Extended Qgis Web Client
 *
 */
var TR = {
    appName: 'Räumliches Informationssystem - ',
    loginTitle: 'Login',
    loginMessage: 'Der Zugriff zu diesem Bereich ist auf authentifizierte Benutzer beschränkt.' +
    '<br />Bitte geben Sie Benutzernamen und Passwort ein.',
    loginFailMessage: 'Einloggen fehlgeschlagen',
    loginWaitMessage: 'Bitte warten...',
    loginButton: 'Einloggen',
    guestButton: 'Gast',
    loginUsernameLabel: 'Benutzername',
    loginPasswordLabel: 'Passwort',
    loginLanguageLabel: 'Sprache',
    loginRememberMeLabel: 'Anmeldung merken',
    loginForgotPasswordLabel: '',
    logoutLabel: 'Ausloggen',
    noProject: 'Das Projekt existiert nicht.',
    noProjectText: 'Füge Sie den richtigen Projektnamen hinzu und laden Sie die Seite erneut.',
    wrongPassword: 'Falsches Passwort.',
    noUser: 'Der Benutzer existiert nicht.',
    noPermission: 'Sie haben keine Berechtigung, um dieses Projekt zu benutzen.',
    noPublicAccess: 'Öffentlicher Zugriff ist für dieses Projekt nicht erlaubt.',
    clearFilter: 'Alle Filter zurücksetzen',
    clearSelection: 'Auswahl aufheben',
    select: 'Select element to display its geometry',
    menuFilterText: 'Filter',
    editData: 'Daten bearbeiten',
    generateReport: 'Word-Bericht generieren',
    editMode: 'Bearbeitungsmodus',
    editDisabled: 'Bearbeiten ist deaktiviert',
    editAdd: 'Hinzufügen',
    editAddOffset: 'Offset',
    editEdit: 'Bearbeiten',
    editDelete: 'Löschen',
    editSave: 'Speichern',
    mapBasic: 'Karte',
    mapSatellite: 'Satellit',
    mapHybrid: 'Hybrid',
    mapTerrain: 'Gelände',
    fiLocation: 'Position',
    fiElevation: 'über dem Meeresspiegel',
    cancel: 'Abbrechen',
    send: 'Senden',
    feedback: 'Feedback',
    feedbackTitle: 'Feedback senden',
    feedbackDescription: 'Name, Project und aktueller Kartenlink werden der Nachricht hinzugefügt!',
    feedbackErrorTitle: "Error",
    feedbackErrorMsg: "There was error sending your message!",
    feedbackSuccessTitle: "Success",
    feedbackSuccessMsg: "Your message was successfully sent.",
    transactionFail: 'Übertragung fehlgeschlagen',
    show: 'Zeige',
    properties: 'Einstellungen',
    description: 'Beschreibung',
    emptyPrintTitleText: 'Titel hinzufügen ...',
    emptyPrintDescriptionText: 'Beschreibung hinzufügen ...',
    loadMore: 'Lade weiteres ...',
    loadMoreToolTip: 'Tabelle ist nicht vollständig. Klicken, um weitere Daten vom Server zu laden!',
    height: 'Höhe',
    exportData: 'Export data',
    exportFormat: 'Format',
    exportCrs: 'CRS',
    exportLayer: 'Layer',
    exportExtent: 'Extent',
    exportLayerExtent: 'Use layer extent',
    exportUseMapCrs: "Use map CRS",
    exportFilter: "Filter",
    exportUseTableFilter: "Use attribute filter set on table",
    tableUseExtent: "Only show features in map extent",
    tableMeasurementFilterApply: "Filter",
    tableMeasurementFilterActive: "Filtered by measurement",
    tableMeasurementFilterClear: "Clear measurement filter",
    tableMeasurementFilterCurrentTable: "Aktuelle Tabelle filtern",
    measurementZoomTooltip: "Zum Element zoomen",
    measurementRemoveTooltip: "Löschen",
    measurementRemoveAllTooltip: "Alle löschen",
    measurementProfileTitle: "Höhenprofil",
    measurementProfilePositionLayer: "Position im Höhenprofil",
    measurementProfileMoveHint: "Bewegen Sie den Mauszeiger über das Profil, um Details anzuzeigen.",
    measurementProfileDistance: "Entfernung",
    measurementProfileElevation: "Höhe",
    measurementProfileNextSlope: "Neigung des nächsten Abschnitts",
    measurementProfileNoData: "k. A.",
    measurementProfileUnavailable: "Die Antwort enthält kein verwendbares Höhenprofil.",
    measurementProfileCreateError: "Das Höhenprofil kann nicht erstellt werden.",
    measurementProfileLoading: "Höhenprofil wird geladen...",
    measurementProfileRequestFailed: "Die Höhenprofilanfrage ist fehlgeschlagen.",
    measurementProfileAriaLabel: "Höhenprofil der Linie",
    tableAddRecord: "Add new record",
    style: "Style",
    navigation: "Navigation",
    relations: "Relations",
    bookmarks: "Lesezeichen",
    bookmarkName: "Name",
    bookmarkGroup: "Gruppe",
    bookmarkEmptyGroupText: "keine Gruppe",
    bookmarkAdd: "Hinzufügen",
    bookmarkEdit: "Bearbeiten",
    bookmarkRemove: "Entfernen",
    bookmarkRemoveConfirm: "Dieses Lesezeichen entfernen?",
    bookmarkSave: "Speichern",
    bookmarkStorageError: "Das Lesezeichen konnte in diesem Browser nicht gespeichert werden.",
    link: "LINK",
    trueText: 'Yes',
    falseText: 'No',
    close: 'Close',
    copyLink: 'Copy link',
    copied: 'Copied'
};

/**
 * Mobile Viewer translation strings
 */

var I18n = {};

I18n.title = "Mobilversion";

I18n.search = {
    header: "Adresssuche",
    results: "Suchergebnis",
    failed: "Suche fehlgeschlagen:",
    placeholder: "Search...",
    noResults: "No results found"
};

I18n.properties = {
    header: "Karteneinstellungen",
    mapFollowing: "Verfolgung",
    mapRotation: "Kartenausrichtung",
    mapLoading: "Karte wird geladen...",
    scaleBar: "Massstab",
    about: "Impressum",
    share: "Share",
    login: "Login",
    on: "Ein",
    off: "Aus"
};

I18n.about = {
    header: "Impressum",
    content: "Versionsnummer"
};

I18n.layers = {
    project: "Projekt",
    topics: "Themen",
    layers: "Ebenen",
    layerOrder: "Layerreihenfolge",
    background: "Hintergrund",
    overlays: "Zusätzliche Themen",
    selection: "Auswahl",
    redlining: "Ausgrenzen",
    transparency: "Transparenz"
};

I18n.featureInfo = {
    header: "Informationen",
    feature: "Feature ID:",
    raster: "Raster",
    noFeatureFound: "Kein Objekt gefunden"
};

I18n.geolocation = {
    permissionDeniedMessage: "Standortabfrage ist deaktiviert.\n\nBitte überprüfen Sie Ihre Browser-Einstellungen, um die Berechtigung zu erhalten.",
    accuracy: "Genauigkeit",
    altitude: "Höhenniveau",
    heading: "Richtung",
    speed: "Geschwindigkeit",
    obtaining: "Standort wird gesucht ...",
    lowAccuracy: "Low Accuracy!"
};

I18n.login = {
    header: "Login",
    user: "Benutzer",
    password: "Passwort",
    signIn: "Anmelden",
    cancel: "Abbrechen",
    signOut: "Abmelden",
    signInFailed: "Benutzername oder Passwort nicht korrekt",
    signOutFailed: "Abmelden fehlgeschlagen",
    statusFailed: "Login Status fehlgeschlagen"
};

I18n.networkDown = 'Internet connection lost';

I18n.measureArea = 'Measure area';