/**
 * Slovene translation for Extended Qgis Web Client
 *
 * Translators: Uros Preloznik
 *
 * Date: 2015-08-17
 */
var TR = {
    appName: 'Prostorski info. sistem - ',
    loginTitle: 'Prijava',
    loginMessage: 'Dostop za pooblaščene osebe.' +
    '<br />Prosimo vnesite uporabniško ime in geslo.',
    loginFailMessage: 'Prijava ni uspela',
    loginWaitMessage: 'Prosimo počakajte...',
    loginButton: 'Prijava',
    guestButton: 'Gost',
    loginUsernameLabel: 'uporabniško ime',
    loginPasswordLabel: 'geslo',
    loginLanguageLabel: 'jezik',
    loginRememberMeLabel: 'zapomni si me na tem računalniku',
    loginForgotPasswordLabel: '',
    logoutLabel: 'odjava',
    noProject: 'Projekt ne obstaja.',
    noProjectText: 'Vnesi ustrezno ime projekta in ponovno naloži stran.',
    wrongPassword: 'Napačno geslo.',
    noUser: 'Uporabnik ne obstaja.',
    noPermission: 'Nimate pravic za uporabo projekta',
    noPublicAccess: 'Javni dostop ni omogočen za ta projekt.',
    clearFilter: 'Pobriši vse filtre',
    clearSelection: 'Pobriši izbor',
    select: 'Izberi element za prikaz geometrije',
    menuFilterText: 'Filtri',
    editData: 'Uredi podatke',
    generateReport: 'Izdelaj poročilo (MS Word format)',
    editMode: 'Urejanje',
    editDisabled: 'Urejanje onemogočeno',
    editAdd: 'Dodaj',
    editAddOffset: 'Odmik',
    editEdit: 'Uredi',
    editDelete: 'Briši',
    editSave: 'Shrani',
    mapBasic: 'Osnovna karta',
    mapSatellite: 'Satelit',
    mapHybrid: 'Hibridni prikaz',
    mapTerrain: 'Topografska karta',
    fiLocation: 'Lokacija',
    fiElevation: 'nadmorska višina',
    cancel: 'Prekini',
    send: 'Pošlji',
    feedback: 'Sporočilo uporabnika',
    feedbackTitle: 'Pošlji svoje sporočilo',
    feedbackDescription: 'Tvoje ime, projekt in trenutna povezava do karte se bodo dodali v sporočilo!',
    feedbackErrorTitle: "Napaka",
    feedbackErrorMsg: "Pošiljanje ni uspelo!!",
    feedbackSuccessTitle: "Uspeh",
    feedbackSuccessMsg: "Sporočilo je bilo poslano.",
    transactionFail: 'Transakcija neuspešna',
    show: 'Prikaži',
    properties: 'Lastnosti',
    description: 'Opis',
    emptyPrintTitleText: 'Dodaj naslov ...',
    emptyPrintDescriptionText: 'Dodaj opis ...',
    loadMore: 'Več...',
    loadMoreToolTip: 'Tabela ni popolna. Klikni za več podatkov iz strežnika!',
    height: 'Višina',
    exportData: 'Izvoz podatkov',
    exportFormat: 'Format',
    exportCrs: 'CRS',
    exportLayer: 'Sloj',
    exportExtent: 'Območje',
    exportLayerExtent: 'Uporabi območje sloja',
    exportUseMapCrs: "Uporabi projektni koord. sistem",
    exportFilter: "Filter",
    exportUseTableFilter: "Uporabi filter tabele",
    tableUseExtent: "Prikaži samo elemente znotraj trenutnega območja",
    tableMeasurementFilterApply: "Filter",
    tableMeasurementFilterActive: "Filtrirano z meritvijo",
    tableMeasurementFilterClear: "Počisti filter meritve",
    tableMeasurementFilterCurrentTable: "Filtriraj trenutno tabelo",
    measurementZoomTooltip: "Prikaži element",
    measurementRemoveTooltip: "Odstrani",
    measurementRemoveAllTooltip: "Odstrani vse",
    measurementProfileTitle: "Profil terena",
    measurementProfilePositionLayer: "Položaj na profilu terena",
    measurementProfileMoveHint: "Premakni kazalec po profilu za prikaz podrobnosti.",
    measurementProfileDistance: "Razdalja",
    measurementProfileElevation: "Višina",
    measurementProfileNextSlope: "Naklon naslednjega odseka",
    measurementProfileNoData: "ni podatka",
    measurementProfileUnavailable: "Odgovor ne vsebuje uporabnega profila terena.",
    measurementProfileCreateError: "Profila terena ni mogoče izdelati.",
    measurementProfileLoading: "Nalaganje profila terena...",
    measurementProfileRequestFailed: "Zahteva za profil terena ni uspela.",
    measurementProfileAriaLabel: "Profil terena linije",
    tableAddRecord: "Dodaj nov zapis",
    style: "Stil",
    navigation: "Navigacija",
    relations: "Relacije",
    bookmarks: "Zaznamki",
    bookmarkName: "naziv",
    bookmarkGroup: "Skupina",
    bookmarkEmptyGroupText: "brez skupine",
    bookmarkAdd: "Dodaj",
    bookmarkEdit: "Uredi",
    bookmarkRemove: "Odstrani",
    bookmarkRemoveConfirm: "Odstrani ta zaznamek?",
    bookmarkSave: "Shrani",
    bookmarkStorageError: "Zaznamka ni bilo mogoče shraniti v ta brskalnik.",
    link: "POVEZAVA",
    trueText: 'da',
    falseText: 'ne',
    close: 'zapri',
    copyLink: 'kopiraj',
    copied: 'Kopirano'
};

/**
 * Mobile Viewer translation strings
 */

var I18n = {};

I18n.title = "Mobilni GIS";

I18n.search = {
    header: "Iskanje",
    results: "Rezultati",
    failed: "Iskanje neuspešno:",
    placeholder: "išči...",
    noResults: "Ni rezultatov"
};

I18n.properties = {
    header: "Nastavitve",
    mapFollowing: "Sledenje",
    mapRotation: "Auto rotacija",
    mapLoading: "Nalaganje karte...",
    scaleBar: "Merilo",
    about: "Imprint",
    share: "Share",
    login: "Login",
    on: "DA",
    off: "NE"
};

I18n.about = {
    header: "Imprint",
    content: "Development version"
};

I18n.layers = {
    project: "Projekt",
    topics: "Teme",
    layers: "Sloji",
    layerOrder: "Možnosti",
    background: "Podlage",
    overlays: "Dodatni sloji",
    selection: "Izbor",
    redlining: "Redlining",
    transparency: "Prosojnost"
};

I18n.featureInfo = {
    header: "Lokacija",
    feature: "Element ID:",
    raster: "Raster",
    noFeatureFound: "Ni elementov"
};

I18n.geolocation = {
    permissionDeniedMessage: "Lokacija ni na voljo.\n\nPreverite nastavitve brskalnika in naprave.",
    accuracy: "Natančnost",
    altitude: "Višina",
    heading: "Smer",
    speed: "Hitrost",
    obtaining: "Pridobivanje lokacije...",
    lowAccuracy: "Slaba natančnost!"
};

I18n.login = {
    header: "Login",
    user: "uporabnik",
    password: "Password",
    signIn: "Login",
    cancel: "Cancel",
    signOut: "Logout",
    signInFailed: "Username or password are wrong",
    signOutFailed: "Login failed",
    statusFailed: "Login Status failed"
};

I18n.networkDown = 'Internet connection lost';

I18n.measureArea = 'Meri površino';