/**
 * English translation for Extended Qgis Web Client
 *
 * Translators: Uros Preloznik
 *
 * Date: 2015-08-17
 */
var TR = {
    appName: 'Spatial info. system - ',
    loginTitle: 'Login',
    loginMessage: 'Access for authorized users.' +
    '<br />Please type your username and password.',
    loginFailMessage: 'Unable to log in',
    loginWaitMessage: 'Please wait...',
    loginButton: 'Login',
    guestButton: 'Guest',
    loginUsernameLabel: 'Username',
    loginPasswordLabel: 'Password',
    loginLanguageLabel: 'Language',
    loginRememberMeLabel: 'Remember me on this computer',
    loginForgotPasswordLabel: '',
    logoutLabel: 'logout',
    noProject: 'Project doesn\'t exist.',
    noProjectText: 'Add correct project name and reload the page.',
    wrongPassword: 'Wrong password.',
    noUser: 'User doesn\'t exist.',
    noPermission: 'You don\'t have permissions to use this project.',
    noPublicAccess: 'Public access is not allowed with this project.',
    clearFilter: 'Clear all filters',
    clearSelection: 'Clear selection',
    select: 'Select element to display its geometry',
    menuFilterText: 'Filters',
    editData: 'Edit data',
    generateReport: 'Generate Word report',
    editMode: 'Edit mode',
    editDisabled: 'Editing disabled',
    editAdd: 'Add',
    editAddOffset: 'Offset',
    editEdit: 'Edit',
    editDelete: 'Delete',
    editSave: 'Save',
    mapBasic: 'Map',
    mapSatellite: 'Satellite',
    mapHybrid: 'Hybrid',
    mapTerrain: 'Terrain',
    fiLocation: 'Location',
    fiElevation: 'above sea level',
    cancel: 'Cancel',
    send: 'Send',
    feedback: 'Feedback',
    feedbackTitle: 'Send your feedback',
    feedbackDescription: 'Your name, project and current map link will be added to the message!',
    feedbackErrorTitle: "Error",
    feedbackErrorMsg: "There was error sending your message!",
    feedbackSuccessTitle: "Success",
    feedbackSuccessMsg: "Your message was successfully sent.",
    transactionFail: 'Transaction failed',
    show: 'Show',
    properties: 'Properties',
    description: 'Description',
    emptyPrintTitleText: 'Add Title ...',
    emptyPrintDescriptionText: 'Add Description ...',
    loadMore: 'Load more...',
    loadMoreToolTip: 'Table is not complete.Click to load more data from server!',
    height: 'Height',
    exportData: 'Export data',
    exportFormat: 'Format',
    exportCrs: 'CRS',
    exportLayer: 'Layer',
    exportExtent: 'Extent',
    exportLayerExtent: 'Use layer extent',
    exportUseMapCrs: "Use map CRS",
    exportFilter: "Filter",
    exportUseTableFilter: "Use attribute filter set on table",
    tableUseExtent: "Only show features in map extent",
    tableMeasurementFilterApply: "Filter",
    tableMeasurementFilterActive: "Filtered by measurement",
    tableMeasurementFilterClear: "Clear measurement filter",
    tableMeasurementFilterCurrentTable: "Filter current table",
    measurementZoomTooltip: "Zoom to element",
    measurementRemoveTooltip: "Remove",
    measurementRemoveAllTooltip: "Remove all",
    measurementProfileTitle: "Elevation profile",
    measurementProfilePositionLayer: "Elevation profile position",
    measurementProfileMoveHint: "Move over the profile to show details.",
    measurementProfileDistance: "Distance",
    measurementProfileElevation: "Elevation",
    measurementProfileNextSlope: "Next segment slope",
    measurementProfileNoData: "n/a",
    measurementProfileUnavailable: "The response does not contain a usable elevation profile.",
    measurementProfileCreateError: "The elevation profile cannot be created.",
    measurementProfileLoading: "Loading elevation profile...",
    measurementProfileRequestFailed: "The elevation profile request failed.",
    measurementProfileAriaLabel: "Line elevation profile",
    tableAddRecord: "Add new record",
    style: "Style",
    navigation: "Navigation",
    relations: "Relations",
    bookmarks: "Bookmarks",
    bookmarkName: "Name",
    bookmarkGroup: "Group",
    bookmarkEmptyGroupText: "no group",
    bookmarkAdd: "Add",
    bookmarkEdit: "Edit",
    bookmarkRemove: "Remove",
    bookmarkRemoveConfirm: "Remove this bookmark?",
    bookmarkSave: "Save",
    bookmarkStorageError: "The bookmark could not be saved in this browser.",
    link: "LINK",
    trueText: 'Yes',
    falseText: 'No',
    close: 'Close',
    copyLink: 'Copy link',
    copied: 'Copied'
};

/**
 * Mobile Viewer translation strings
 */

var I18n = {};

I18n.title = "Mobile Viewer";

I18n.search = {
    header: "Search",
    results: "Results",
    failed: "Search failed:",
    placeholder: "Search...",
    noResults: "No results found"
};

I18n.properties = {
    header: "Map Settings",
    mapFollowing: "Tracking",
    mapRotation: "Auto Rotation",
    mapLoading: "Loading Map...",
    scaleBar: "Scale",
    about: "Imprint",
    share: "Share",
    login: "Login",
    on: "On",
    off: "Off"
};

I18n.about = {
    header: "Imprint",
    content: "Development version"
};

I18n.layers = {
    project: "Project",
    topics: "Themes",
    layers: "Layers",
    layerOrder: "Layer order",
    background: "Background",
    overlays: "Overlays",
    selection: "Selection",
    redlining: "Redlining",
    transparency: "Transparency"
};

I18n.featureInfo = {
    header: "Information",
    feature: "Feature ID:",
    raster: "Raster",
    noFeatureFound: "No objects found"
};

I18n.geolocation = {
    permissionDeniedMessage: "Location is not available.\n\nPlease check your browser or device settings.",
    accuracy: "Accuracy",
    altitude: "Altitude",
    heading: "Heading",
    speed: "Speed",
    obtaining: "Obtaining location...",
    lowAccuracy: "Low Accuracy!"
};

I18n.login = {
    header: "Login",
    user: "User",
    password: "Password",
    signIn: "Login",
    cancel: "Cancel",
    signOut: "Logout",
    signInFailed: "Username or password are wrong",
    signOutFailed: "Login failed",
    statusFailed: "Login Status failed"
};

I18n.networkDown = 'Internet connection lost';

I18n.measureArea = 'Measure area';